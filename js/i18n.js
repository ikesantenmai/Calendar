...
(function (global) {
  ...
  var DICT = {
    ja: {
      ...
      'imp.load': '読み込む',
      'imp.corsNote': '配信元が許可していない場合はブラウザの制限（CORS）で取得できません。その場合はファイルを保存してから選んでください。',
      'imp.preview': '読み込み内容',
      'imp.name': 'カレンダー名',
      'imp.color': '色',
      'imp.target': '取り込み先',
      ...
      'imp.example': '例: {list}',
      'imp.loading': '読み込み中…',
      'imp.fetchError': 'URL から取得できませんでした（{msg}）。ブラウザの制限（CORS）が原因のことがあります。URL をブラウザで直接開いて .ics を保存し、この画面で選んでください。',
    ...
    en: {
      ...
      'imp.load': 'Load',
      'imp.corsNote': 'If the publisher does not allow it, the browser blocks the request (CORS). Save the file first and choose it here instead.',
      ...
      'imp.loading': 'Loading…',
      'imp.fetchError': 'Could not load that URL ({msg}). This is often a browser restriction (CORS). Open the URL directly, save the .ics file and choose it here.',
